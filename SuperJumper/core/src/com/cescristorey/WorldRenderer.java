...
public class WorldRenderer {
	...
	private void renderSquirrels () {
		...
		int len = world.personajes.size();
		for (int i = 0; i < len; i++) {
			Personaje personaje = world.personajes.get(i);
			Texture keyFrame = Assets.person;
			float side = personaje.velocity.x < 0 ? -1 : 1;
			if (side < 0)
				batch.draw(keyFrame, personaje.position.x + 0.5f, personaje.position.y - 0.5f, side * 1, 1);
			else
				batch.draw(keyFrame, personaje.position.x - 0.5f, personaje.position.y - 0.5f, side * 1, 1);
		}
	}
