...
public class World {
	...
	public final List<Coin> coins;
        public final List<Personaje> personajes;
	...
	private void generateLevel () {
		...
		while (y < WORLD_HEIGHT - WORLD_WIDTH / 2) {
			...
			if (rand.nextFloat() > 0.6f) {
				...
				Personaje personaje = new Personaje(platform.position.x + rand.nextFloat(), platform.position.y + Personaje.COIN_HEIGHT
					+ rand.nextFloat() * 3);
	...
	private void updateCoins (float deltaTime) {
        ...
        private void updatePersonajes (float deltaTime) {
		int len = personajes.size();
		for (int i = 0; i < len; i++) {
	...
	private void checkItemCollisions () {
        ...
        private void checkMariposaCollisions () {
                int lol = personajes.size();
		for (int i = 0; i < lol; i++) {
			Personaje personaje = personajes.get(i);
			if (bob.bounds.overlaps(personaje.bounds)) {
				personajes.remove(personaje);
				lol = personajes.size();
				listener.coin();
				score += Personaje.COIN_SCORE;
			}
